...
class HoughLines():
    ...
    def __compute_normal_vectors__(self, degrees):
        ...
        for index, thetaDegrees in enumerate(degrees):
            theta_rad = np.deg2rad(thetaDegrees)
            nx = np.cos(theta_rad)  # n = [cos(θ), sin(θ)]
            ny = np.sin(theta_rad)
            normalVectors[index, :] = np.array([nx, ny])
        return normalVectors


    def __vote_from_pixel__(self, imageCoordinateXY: np.ndarray, grayValue: int):
        x, y = imageCoordinateXY.astype(float)

        for angleDegrees in np.arange(0, self.maxThetaDegrees):
            angleIndex = self.indexFromAngle[angleDegrees]
            n_current = self.normalVectors[angleIndex,:]

            # Compute radius ρ = x·cosθ + y·sinθ
            rho = x * n_current[0] + y * n_current[1]
            rho_rounded = int(np.round(rho))

            # in valid range
            rho_rounded = np.clip(rho_rounded, self.minRadius, self.maxRadius)
            radiusIndex = self.indexFromRadius[rho_rounded]

            # Add weighted vote (gray value) in accumulator
    ...
    def __draw_found_line__(self, img, houghPeakCoordinates, halfLength = 200, color=(255,255,255)):
        radiusIndex = int(houghPeakCoordinates[0])
        angleIndex = int(houghPeakCoordinates[1])
        rho = self.radiusFromIndex[radiusIndex]
        theta_deg = self.angleFromIndex[angleIndex]
        theta_rad = np.deg2rad(theta_deg)

        n = np.array([np.cos(theta_rad), np.sin(theta_rad)], dtype=float)

        # line direction vector u (orthogonal to n) --> rotation by 90°
        u = np.array([-n[1], n[0]], dtype=float)

        point_on_line = n * rho

        startPoint = point_on_line - halfLength * u
        endPoint = point_on_line + halfLength * u
