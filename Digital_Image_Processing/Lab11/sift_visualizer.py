...
class SiftVisualizer():
    ...
    def img_sift_descriptor(self, img, key, descr, id, color=(255, 255, 255), zoom_factor=1):
        ...
        for field in range(0, HOGS):
            for dir in range(0, DIRS):
                ...
                cv2.line(img, tuple(origin.astype(np.int32)), tuple(target.astype(np.int32)),
    ...
    def plot_lines_on_image(self, img, begin_pt, end_pt, color=(255, 255, 255)):
        ...
        for n in range(0, N):
            pt1 = tuple(begin_pt[:, n].astype(np.int32))
            pt2 = tuple(end_pt[:, n].astype(np.int32))
    ...
    def img_sift_keypoint(self, img, key, descr, id, color=(255, 255, 255), zoom_factor=1):
        ...
        for rot in range(0, 4):
            ...
            line_points = rot_points.astype(np.int32) + np.repeat(np.array(center).reshape(2, 1), 2, axis=1)
        ...
        bullet_center = tuple((rotmat_angle @ bullet_center).astype(np.int32) + np.array(center))
